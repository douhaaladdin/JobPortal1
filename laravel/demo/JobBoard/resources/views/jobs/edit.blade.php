...
<div class="container">
    ...
    <form action="{{ route('jobs.update', $job->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea name="responsibilities" class="form-control" rows="3" required>{{ $job->responsibilities }}</textarea>
        </div>

        <div class="mb-3">
            <label class="form-label">Required Skills</label>
            <input type="text" name="required_skills" class="form-control" value="{{ $job->required_skills }}" required>
        </div>
        ...
        <div class="mb-3">
            ...
            @if($job->company_logo)
                <img src="{{ asset('storage/'.$job->company_logo) }}" width="100" class="mt-2">
            @endif
        </div>
